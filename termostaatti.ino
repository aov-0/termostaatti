...
int ASETUS;
int OHJAUS;
int TAULU[5];     //Muuttuja keskiarvoistamista varten. Varaa 5 tallennuspaikkaa tämän käyttöön.
int INDEX = 0;
...
void setup(){
  ...
  for(int i = 0; i<5; i++){           //for silmukassa "i" muuttujaan tallennetaan i:n arvoksi 0, ja niin kauan kunnes i:n arvo on 5, lisätään lukemaan joka kierron jälkeen +1.
   TAULU[i] =  analogRead(ANTURI);    //Lukee anturin arvon, tallentaa TAULU muuttujaan. Tässä tapahtuu keskiarvoistus. Tehdään siksi ettei puhallin lähde heti alkuun pyörimään.
  }
}
...
void loop() {
  ...
  Serial.print(ASETUS/100.0);               //ASETUS arvo jaetaan sadalla koska se aiemmin mäpättiin sata kertaiseksi lisätarkkuuden saavuttamiseksi. Tässä haluttiin tulostaa arvo sarjaporttiin jotta voitiin tarkkailla arvoja koodauksen aikana. Lisättäessä piste luvun perään, saadaan sarjaporttiin desimaalilukuja.
  Serial.println(" astetta");
  ...
  }else{                      //Muutoin puhallin laitetaan pois päältä.
    analogWrite(FAN, 0);
  }
